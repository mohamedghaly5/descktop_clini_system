...
export const licenseService = {
    ...
    getInternalState() {
        ...
        let licenseType = appMetaService.get('license_type') || 'Trial';
        const currentFp = this.getFingerprint();
    ...
    async activateLicense(keyInput) {
        try {
            ...
            const myDevice = registeredDevices.find((d) => d.device_fingerprint === currentFp);
            if (myDevice) {
    ...
    enableSupportAccess(durationHours = 24) {
        const now = new Date();
    ...
    isWriteAllowed() {
        ...
        try {
            const db = getDb();
            const settings = db.prepare('SELECT is_setup_completed FROM clinic_settings LIMIT 1').get();
