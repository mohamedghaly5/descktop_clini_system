...
export function initializeDatabase() {
  ...
  `;
    ...
    const clinicSettings = db.prepare('SELECT count(*) as count FROM clinic_settings').get();
    ...
    // Backfill Patients
    const patientsWithoutId = db.prepare('SELECT id FROM patients WHERE display_id IS NULL ORDER BY created_at ASC').all();
    ...
    // 2. Treatment Cases
    const caseCols = db.prepare('PRAGMA table_info(treatment_cases)').all();
    ...
    const casesWithoutId = db.prepare('SELECT id FROM treatment_cases WHERE display_id IS NULL ORDER BY created_at ASC').all();
    if (casesWithoutId.length > 0) {
        let maxId = db.prepare('SELECT MAX(display_id) as max FROM treatment_cases').get()?.max || 0;
    ...
    // Backfill Invoices
    const invoicesWithoutId = db.prepare('SELECT id FROM invoices WHERE display_id IS NULL ORDER BY created_at ASC').all();
    if (invoicesWithoutId.length > 0) {
        let maxId = db.prepare('SELECT MAX(display_id) as max FROM invoices').get()?.max || 0;
