...
export function validateSchema() {
    ...
    try {
        if (allGood) {
            ...
            const invoiceCount = db.prepare('SELECT COUNT(*) as c FROM invoices').get();
            ...
            const orphans = db.prepare("SELECT COUNT(*) as c FROM invoices WHERE clinic_id IS NULL OR clinic_id = ''").get();
            if (orphans.c > 0) {
