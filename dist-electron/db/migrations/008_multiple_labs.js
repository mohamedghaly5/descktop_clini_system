...
export const up = (db) => {
    ...
    const existing = db.prepare("SELECT id FROM labs WHERE is_default = 1").get();
    let defaultLabId;
    if (existing) {
    ...
    const servicesCols = db.prepare("PRAGMA table_info(lab_services)").all();
    ...
    // 4. Modify lab_orders
    const ordersCols = db.prepare("PRAGMA table_info(lab_orders)").all();
