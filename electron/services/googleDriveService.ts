...
export class GoogleDriveService {
    ...
    async startAuthFlow(): Promise<string> {
        if (!this.oAuth2Client) throw new Error('Client not initialized');
    ...
    private async ensureBackupFolder(drive: any) {
        // Search for folder
    ...
    async uploadFile(filePath: string, fileName: string, mimeType: string, description?: string) {
        await this.authenticate();
        const drive = google.drive({ version: 'v3', auth: this.oAuth2Client });
    ...
    async findFile(name: string, folderId?: string) {
        await this.authenticate();
        const drive = google.drive({ version: 'v3', auth: this.oAuth2Client });
    ...
    async listFiles(limit: number = 20) {
        try {
            await this.authenticate();
            const drive = google.drive({ version: 'v3', auth: this.oAuth2Client });
            const folderId = await this.ensureBackupFolder(drive);
