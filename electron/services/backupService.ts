...
export class BackupService {
    ...
    private resolveDatabasePath(): string {
        ...
        if (fs.existsSync(rootAltPath)) return rootAltPath;
    ...
    async performBackup(options: { password?: string, mode?: 'local' | 'cloud' | 'both' } = {}) {
        try {
            ...
            if (mode === 'cloud' || mode === 'both') {
                ...
                if (isAuth) {
                    ...
                    cloudResult = await googleDriveService.uploadFile(
                        processPath,
                        'dental_clinic_backup.db',
                        'application/octet-stream', // Binary stream is safe for both
                        description
                    );
                }
            }
            ...
            if (mode === 'local' || mode === 'both') {
                const localPath = store.get(LOCAL_BACKUP_PATH_KEY) as string;
            ...
            // Cleanup temp
            if (fs.existsSync(processPath)) fs.unlinkSync(processPath);
    ...
    verifyBackupOwnership(backupPath: string, emailToVerify: string | null, force: boolean = false): boolean {
        let db: any;
        try {
            ...
            if (error.message.startsWith('SECURITY_REQUIRED')) throw error;
            if (error.message.startsWith('SECURITY_MISMATCH')) throw error;
    ...
    private verifyDatabaseIntegrity(dbPath: string): boolean {
        let tempDb: any;
        try {
    ...
    private restoreLicense(data: any) {
        if (!data) return;
        try {
            ...
            if (data.cache) appMetaService.set('license_cache_encrypted', data.cache);
            if (data.keyMasked) appMetaService.set('license_key_masked', data.keyMasked);
            if (data.expiresAt) appMetaService.set('license_expires_at', data.expiresAt);
            if (data.lastCheck) appMetaService.set('last_license_check_at', data.lastCheck);
            if (data.fingerprint) appMetaService.set('device_fingerprint', data.fingerprint);
            if (data.unlockUntil) appMetaService.set('support_unlock_until', data.unlockUntil);
            if (data.status) appMetaService.set('license_status', data.status);
            if (data.type) appMetaService.set('license_type', data.type);
            if (data.ownerEmail) appMetaService.set('owner_email', data.ownerEmail);
        } catch (e) {
     ...
     */
    private sanitizeDatabase(targetDbPath: string) {
        let db: any;
        try {
    ...
    private async rollbackDatabase(originalDbPath: string, recoveryPath: string) {
    ...
    async restoreFromCloud(fileId?: string, password?: string, explicitDbPath?: string, verifyEmail?: string, force: boolean = false) {
        let downloadPath = '';
        let decryptedPath = '';
        let recoveryPath = '';
        ...
        try {
            const isAuth = await googleDriveService.isAuthenticated();
            if (!isAuth) throw new Error('Not authenticated with Google Drive');
            ...
            if (!targetFileId) {
                ...
                if (!file || !file.id) throw new Error('No backup file found on Drive');
            ...
            try {
                ...
                if (meta && meta.description) {
                    ...
                    if (desc.encrypted) metadataEncrypted = true;
                }
            } catch (e) { /* ignore parse error */ }

            if (metadataEncrypted && !password) throw new Error('SECURITY_PASSWORD_REQUIRED');
            ...
            // Cleanup
            if (fs.existsSync(downloadPath)) fs.unlinkSync(downloadPath);
            if (decryptedPath && fs.existsSync(decryptedPath)) fs.unlinkSync(decryptedPath);
    ...
    async restoreFromLocalFile(filePath: string, password?: string, explicitDbPath?: string, verifyEmail?: string) {
        let decryptedPath = '';
        let recoveryPath = '';
        const dbPath = explicitDbPath ? explicitDbPath : this.getDbPath();
        const savedLicense = this.backupCurrentLicense();
        ...
        try {
            ...
            // 2. Password Check
            if (isEncrypted && !password) throw new Error('SECURITY_PASSWORD_REQUIRED');
            ...
            if (fs.existsSync(dbPath)) {
                try {
                    recoveryPath = path.join(path.dirname(dbPath), 'auto_recovery.db');
                    fs.copyFileSync(dbPath, recoveryPath);
                } catch (e) { console.warn('[Restore] Failed to create auto-recovery backup', e); }
            }
            ...
            if (password && fs.existsSync(decryptedPath)) fs.unlinkSync(decryptedPath);
    ...
    private pruneLocalBackups(dir: string) {
    ...
    private async pruneCloudBackups() {
        try {
            const files = await googleDriveService.listFiles(50); // Fetch more to check retention
            const backupFiles = files.filter((f: any) => f.name.startsWith('dental_backup_'));
    ...
    setLocalPath(p: string) {
        store.set(LOCAL_BACKUP_PATH_KEY, p);
    }

    async checkAndRunScheduledBackup() {
        try {
            const frequency = store.get('backup_frequency', 'off') as string;
            if (frequency === 'off') return;

            const lastBackupStr = store.get('last_backup_timestamp') as string;
