...
export function registerAuthHandlers() {
    ...
    try {
        ...
        const adminUser = db.prepare("SELECT id, name, pin_code FROM users WHERE role = 'admin'").get() as any;
        ...
        if (!userCols.some(c => c.name === 'username')) {
            try { db.prepare('ALTER TABLE users ADD COLUMN username TEXT UNIQUE').run(); } catch (e) { }
        }
    ...
    ipcMain.handle('auth:get-users', () => {
        try {
            ...
            const users = db.prepare("SELECT id, name, role, clinic_id FROM users WHERE active = 1").all() as any[];
            ...
            if (e.message === 'DB_NOT_OPEN') return [];
    ...
    ipcMain.handle('auth:login', (_, { userId, pin, remember }) => {
        try {
            ...
            if (error.message === 'DB_NOT_OPEN') return { success: false, error: 'Database not ready' };
    ...
    ipcMain.handle('auth:check', () => {
        ...
        try {
            ...
            const db = getDb();
            const user = db.prepare("SELECT * FROM users WHERE id = ? AND active = 1").get(currentUserId) as any;
            if (!user) {
    ...
    ipcMain.handle('auth:change-pin', (_, { oldPin, newPin }) => {
        try {
            ...
            if (oldHash !== user.pin_code) return { success: false, error: 'Incorrect current PIN' };

            // Basic validation
            if (newPin.length < 4) return { success: false, error: 'PIN must be at least 4 digits' };
            if (newPin === '0000') return { success: false, error: 'Cannot use default PIN' };
    ...
    ipcMain.handle('auth:create-initial-admin', (_, { name, pin }) => {
        try {
            ...
            const anyUser = db.prepare('SELECT count(*) as count FROM users').get() as { count: number };
            if (anyUser.count > 0) {
    ...
    ipcMain.handle('auth:change-password', (_, { currentPassword, newPassword, confirmNewPassword }) => {
        try {
            ...
            // Validation
            if (!newPassword || newPassword.trim() === '') return { success: false, error: 'New password cannot be empty' };
            if (newPassword !== confirmNewPassword) return { success: false, error: 'New passwords do not match' };
            ...
            if (currentHash !== user.pin_code) return { success: false, error: 'Incorrect current password' };
    ...
    ipcMain.handle('auth:check-admin-exists', () => {
        try {
            ...
            if (e.message === 'DB_NOT_OPEN') return false;
            ...
            const transaction = db.transaction(() => {
                ...
                if (permissions && permissions.length > 0) {
                    ...
                    for (const code of permissions) {
                        const perm = getPermId.get(code) as any;
                        if (perm) {
    ...
    ipcMain.handle('users:update-permissions', (_, { userId, permissions }) => {
        try {
            ...
            const requester = db.prepare("SELECT * FROM users WHERE id = ?").get(currentUserId) as any;
