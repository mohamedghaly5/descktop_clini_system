...
export function registerAuthHandlers() {
    ...
    try {
        ...
        const adminUser = db.prepare("SELECT id FROM users WHERE role = 'admin' AND (pin_code IS NULL OR pin_code = '')").get() as any;
        if (adminUser) {
    ...
    ipcMain.handle('auth:create-initial-admin', (_, { name, pin }) => {
        try {
            ...
            try {
                const setting = db.prepare('SELECT id FROM clinic_settings LIMIT 1').get() as { id: string };
                if (setting) clinicId = setting.id;
            } catch { }
