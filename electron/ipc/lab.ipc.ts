...
export function registerLabHandlers() {
    ...
    ipcMain.handle('lab:delete-lab', (_, labId) => {
        ...
        try {
            // 1. Check if default
            const lab = getDb().prepare(`SELECT is_default FROM labs WHERE id = ?`).get(labId) as { is_default: number };
            if (!lab) return { success: false, error: 'Lab not found' };
            if (lab.is_default) return { success: false, error: 'Cannot delete the default lab. Please set another lab as default first.' };

            // 2. Check for linked services
            const serviceCount = getDb().prepare(`SELECT count(*) as count FROM lab_services WHERE lab_id = ?`).get(labId) as { count: number };
            if (serviceCount.count > 0) return { success: false, error: 'Cannot delete lab because it has linked services. Delete or reassign services first.' };

            // 3. Check for linked orders
            const orderCount = getDb().prepare(`SELECT count(*) as count FROM lab_orders WHERE lab_id = ?`).get(labId) as { count: number };
            if (orderCount.count > 0) return { success: false, error: 'Cannot delete lab because it has existing orders.' };
    ...
    ipcMain.handle('lab:get-services', (_, labId?: string) => {
        try {
            const clinicId = getCurrentClinicId();
            ...
            getDb().transaction(() => {
                ...
                if (paidAmount > 0) {
                    ...
                    const orderDetails = getDb().prepare(`
                        ...
                        WHERE lo.id = ?
                    `).get(orderId) as { patient_name: string, service_name: string };
        ...
        try {
            getDb().transaction(() => {
                ...
                if (deleteExpenses) {
                    ...
                    `).all(orderId) as { expense_id: string }[];
    ...
    ipcMain.handle('lab:create-general-payment', (_, { labId, amount, notes, paymentDate }) => {
        ...
        const lab = getDb().prepare('SELECT name FROM labs WHERE id = ?').get(labId) as { name: string };
        ...
        try {
            getDb().transaction(() => {
                ...
                `).all(labId, clinicId) as { id: string, total_lab_cost: number, paid: number }[];
